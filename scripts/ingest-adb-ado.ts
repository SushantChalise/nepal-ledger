/**
 * Ingest CLI for ADB Asian Development Outlook (Nepal chapter).
 *
 * SOURCE_ID: "adb-ado-nepal" | Parser: scrapers/adb_ado/parser.py v0.1.0
 *
 * No default input — download the ADB ADO Nepal chapter PDF from
 * https://www.adb.org/countries/nepal/economy and supply via --input.
 *
 * Usage:
 *   pnpm ingest:adb-ado --dry-run --input "path/to/ado-nepal-2024.pdf"
 *   pnpm ingest:adb-ado --input "path/to/ado-nepal-2024.pdf"
 */
...
const DEFAULT_SOURCE_ID = 'adb-ado-nepal';
const PARSER_PATH = path.join(REPO_ROOT, 'scrapers', 'adb_ado', 'parser.py');
...
function log(msg: string): void {
  process.stdout.write(`[ingest-adb-ado] ${msg}\n`);
}

function logErr(msg: string): void {
  process.stderr.write(`[ingest-adb-ado] ${msg}\n`);
}
...
async function main(): Promise<void> {
  ...
  if (!args.inputPath) {
    logErr(
      '--input is required: download the ADB ADO Nepal chapter PDF from https://www.adb.org/countries/nepal/economy',
    );
    process.exit(2);
  }
