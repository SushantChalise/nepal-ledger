/**
 * Ingest CLI for the NRB NCPI Table 2(B) CSV.
 *
 * SOURCE_ID: "nrb-ncpi-table" (declared in scrapers/nrb_ncpi/parser.py
 * as SOURCE_ID = "nrb-ncpi-table"). The parser reads the tabular CSV at
 ...
 *   4. Spawn scrapers/nrb_ncpi/parser.py subprocess
 ...
 *   pnpm ingest:ncpi --input "NRB Current/CMEFs_Table_Nine-Months_2082.83(2(B).csv"
 *   pnpm ingest:ncpi --source-id nrb-ncpi-table
 *
 * The script must be run from the repo root (where node_modules lives):
 *   cd /path/to/Economy && node_modules/.bin/tsx .claude/worktrees/.../scripts/ingest-ncpi.ts
...
// REPO_ROOT is the worktree root — the scrapers live here.
const SCRIPT_DIR = path.dirname(fileURLToPath(import.meta.url));
const REPO_ROOT = path.resolve(SCRIPT_DIR, '..');

// SOURCE_ID from scrapers/nrb_ncpi/parser.py: SOURCE_ID = "nrb-ncpi-table"
const DEFAULT_SOURCE_ID = 'nrb-ncpi-table';

// The NCPI Table 2(B) CSV. Data files live at the project root
// (parent of .claude/worktrees/<worktree>).
const PROJECT_ROOT = path.resolve(REPO_ROOT, '..', '..', '..');
const DEFAULT_INPUT_RELATIVE = path.join('NRB Current', 'CMEFs_Table_Nine-Months_2082.83(2(B).csv');
...
const PARSER_PATH = path.join(REPO_ROOT, 'scrapers', 'nrb_ncpi', 'parser.py');
...
function log(msg: string): void {
  process.stdout.write(`[ingest-ncpi] ${msg}\n`);
}

function logErr(msg: string): void {
  process.stderr.write(`[ingest-ncpi] ${msg}\n`);
}
