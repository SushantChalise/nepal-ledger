/**
 * Ingest CLI for the NRB CMEFs English-edition PDF.
 *
 * SOURCE_ID: "nrb-cmefs-monthly" (declared in scrapers/nrb_cmefs/parser.py
 * as SOURCE_ID = "nrb-cmefs-monthly"). Note: the task brief mentions the
 * CSV file NRB Current/CMEFs_Table_Nine-Months_2082.83(2(B).csv as the
 * default input, but that file is an NRB tabular CSV — the parser at
 * scrapers/nrb_cmefs/parser.py is a PDF narrative parser (uses pdfplumber).
 * The correct default input is the English CMEFs PDF; the CSV default in
 * the brief appears to be a copy-paste error from an earlier task brief.
 *
 ...
 *   4. Spawn scrapers/nrb_cmefs/parser.py subprocess
 ...
 *   pnpm ingest:cmefs --input "Stastical Information/CMEFs_Eng_Nine-Months_2082.83.pdf"
 *   pnpm ingest:cmefs --source-id nrb-cmefs-monthly
 *
 * The script must be run from the repo root (where node_modules lives):
 *   cd /path/to/Economy && node_modules/.bin/tsx .claude/worktrees/.../scripts/ingest-cmefs.ts
...
// SOURCE_ID from scrapers/nrb_cmefs/parser.py: SOURCE_ID = "nrb-cmefs-monthly"
const DEFAULT_SOURCE_ID = 'nrb-cmefs-monthly';

// The English CMEFs PDF — the narrative prose parser (pdfplumber) reads this.
// The CSV at "NRB Current/CMEFs_Table_Nine-Months_2082.83(2(B).csv" is a
// different file format and would require a separate tabular parser.
// Data files live at the project root (parent of .claude/worktrees/<worktree>).
const PROJECT_ROOT = path.resolve(REPO_ROOT, '..', '..', '..');
const DEFAULT_INPUT_RELATIVE = path.join(
  'Stastical Information',
  'CMEFs_Eng_Nine-Months_2082.83.pdf',
);
...
const PARSER_PATH = path.join(REPO_ROOT, 'scrapers', 'nrb_cmefs', 'parser.py');
...
function log(msg: string): void {
  process.stdout.write(`[ingest-cmefs] ${msg}\n`);
}

function logErr(msg: string): void {
  process.stderr.write(`[ingest-cmefs] ${msg}\n`);
}
...
async function main(): Promise<void> {
  ...
  if (args.dryRun) {
    ...
    await new Promise<void>((resolve, reject) => {
      const child = spawn(
        python,
        [PARSER_PATH, args.inputPath, placeholderDocId],
  ...
  const result = await ingestSource({
    ...
    reportingPeriodLabel: 'FY 2082/83 9M',
  ...
  const approvedTotal =
    summary.validation.promoted + summary.validation.promotedWithWarnings;
  ...
  // scripts per existing patterns (ingest-fiscal-transfers.ts §persist).
  if (approvedTotal > 0) {
    log('');
