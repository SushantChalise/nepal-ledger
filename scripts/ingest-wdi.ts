/**
 * Ingest CLI for World Bank WDI Nepal macro benchmark snapshots.
 *
 * SOURCE_ID: "wb-wdi" | Parser: scrapers/wb_wdi/parser.py v0.1.0
 *
 * Two-step process:
 *   1. Download snapshot: python -m scrapers.wb_wdi.fetch --output snapshot.json
 *   2. Ingest snapshot:   pnpm ingest:wdi --input snapshot.json
 *
 * Usage:
...
const DEFAULT_SOURCE_ID = 'wb-wdi';
const PARSER_PATH = path.join(REPO_ROOT, 'scrapers', 'wb_wdi', 'parser.py');
...
async function main(): Promise<void> {
  ...
  if (!args.inputPath) {
    logErr(
      '--input is required: first generate a snapshot with ' +
        '`python -m scrapers.wb_wdi.fetch --output snapshot.json`',
    );
    process.exit(2);
  }
