"""Download a catalog ``ReportRef`` to a content-addressable local archive.
...
beyond bytes-on-disk + Content-Type. Mirrors nso_archive.archive.
"""
...
def _write_sidecar(sidecar_path: Path, ref: ReportRef, archived: ArchivedReport) -> None:
    payload = {
        "schema_version": "1",
        "ref": asdict(ref),
        ...
        try:
            with http.stream("GET", ref.url, timeout=timeout_s) as resp:
                ...
                fd, tmp_name = tempfile.mkstemp(
                    prefix=".oag-", suffix=".pdf.part", dir=str(output_dir)
                )
