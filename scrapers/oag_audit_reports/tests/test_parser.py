"""Tests for oag_audit_reports.parser — pure table-row logic.

Fixtures are the REAL extracted tables from the 58th Annual Report (English,
FY 2076/77), pages 28 (audited entities) and 33 (irregularity by classification
x tier), captured via pdfplumber. The pure functions are exercised without a
PDF; figures are asserted against docs/research/oag-audit-reports-audit.md.
"""
...
# --- Chapter 2 continuation, page 34: printed total row + settlement table ----
# pdfplumber splits the Ch.2 table across the page break; the printed per-class
# "Total irregularity" row lands in the continuation table.
...
def test_parse_ch1_audited_per_class() -> None:
    ...
    # 5 financial-audit subject classes; "(B) Performance Audit" is not a class.
    assert set(by_class) == {
...
def test_parse_ch2_beruju_recoverable_row() -> None:
    cells = parse_ch2_beruju(CH2_ROWS)
    rec = {c.subject_class: c for c in cells if c.category == "recoverable"}
...
def test_parse_ch2_skips_parents_and_subrows() -> None:
    cells = parse_ch2_beruju(CH2_ROWS)
    labels = {c.category_label_raw for c in cells}
    # "2. To be regularized" parent and "3.1 Staff Advance" sub-row are excluded.
    assert "2. To be regularized" not in labels
    assert "3.1 Staff Advance" not in labels
    # The "3. Advance" PARENT total IS emitted (its sub-rows roll up into it).
    assert "3. Advance" in labels
    cats = {c.category for c in cells}
    assert cats == {
        "recoverable", "irregular", "evidence_not_submitted",
        "responsibility_not_transferred", "revenue_arrears", "advance_outstanding",
    }
...
def test_parse_ch2_column_alignment_with_blank_cells() -> None:
    ...
    cells = parse_ch2_beruju(CH2_ROWS)
    bal = {c.subject_class: c for c in cells if c.category == "responsibility_not_transferred"}
...
def test_beruju_reconciles_to_printed_grand_total() -> None:
    # Leaf categories must sum to the report's stated NRs 104,384.3 million.
    cells = parse_ch2_beruju(CH2_ROWS)
    total = sum((c.amount_npr for c in cells), Decimal(0))
    assert total == Decimal("104384.3") * _M


def test_beruju_reconciles_per_class() -> None:
    cells = parse_ch2_beruju(CH2_ROWS)
    per_class: dict[str, Decimal] = {}
    for c in cells:
        per_class[c.subject_class] = per_class.get(c.subject_class, Decimal(0)) + c.amount_npr
    # Recon doc: federal 44.39 / provincial 6.50 / local 40.83 (billions).
...
def test_parse_ch2_total_row() -> None:
    ...
    assert totals["local_government"].npr == Decimal("40834.7") * _M
    assert set(totals) == {
...
def test_parse_settlement_table_maps_committee_not_provincial() -> None:
    ...
    # "Other Committee/Institution (Including Provincial)" must map to committee,
    # NOT provincial, despite containing the word "Provincial".
    ...
    # settled = "Last Year's Cleared/Settled"; cumulative = "Cumulative Outstanding".
    ...
    # The "Total" row is not a class and is skipped.
...
def test_build_output_contract_shape() -> None:
    ...
    assert out["status"] == "success"  # category lines reconcile to printed totals
    ...
    # Every promoted row carries required provenance (Parser Ship Gate).
    for row in out["summaries"] + out["beruju_lines"]:
        assert row["confidence_grade"] == "A"
        assert row["source_document_id"] == "11111111-1111-4111-8111-111111111111"
        assert row["extraction_method"] == "text_layer"
    for line in out["beruju_lines"]:
        assert line["amount_raw"]  # non-null amount keeps its raw expression
        assert line["source_unit"] == "NPR_million"
        assert line["amount_basis"] == "current_year_raised"

    # Federal: printed beruju_raised + raw, plus settled + cumulative scalars.
    ...
    # Raw-when-amount invariant: every non-null *_npr has its *_raw.
    ...
    # Corporate bodies appear in Ch.1 (audited) but not the Ch.2 irregularity
    # table -> beruju_raised is null, audited is present.
...
def test_build_output_flags_reconciliation_variance() -> None:
    # Corrupt one category cell so the federal column no longer sums to its
    # printed total -> a ReconciliationFailed error and status 'partial'.
    bad_rows = [list(r) for r in CH2_ROWS]
    bad_rows[3][1] = "20,292.9"  # Recoverable / Federal: 20,292.1 -> 20,292.9
    out = build_output(
        fiscal_year_bs="2076/77",
        source_document_id="11111111-1111-4111-8111-111111111111",
        ch1=parse_ch1_audited(CH1_ROWS),
        ch2=parse_ch2_beruju(bad_rows),
        totals=parse_ch2_total_row(CH2_CONTINUATION_ROWS),
        settlement=parse_settlement_table(SETTLEMENT_ROWS),
    )
