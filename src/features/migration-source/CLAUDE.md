...
**View C — origin→destination flow Sankey (Atlas Fig 6).** The absent population traced from origin **province** to destination **region** (`MigrationFlowSankey`), rolled up from palika-grain census via the authoritative `DISTRICT_TO_PROVINCE` map (`district-province.ts`, derived from the source geometry's province field — not guessed). Reuses the `src/lib/viz/adapters/d3-sankey.ts` adapter (ADR-0012); mirrors `money-flow`'s Sankey field-for-field (mobile stacked-bar fallback + sr-only table). 13 census destination buckets are consolidated to the 6 the Atlas's flow uses. Unit is **people**.
...
- `components/MigrationFlowSankey.tsx` (View C) — `'use client'`; province→region Sankey via `computeSankeyLayout` + `sankeyLinkHorizontal` from `src/lib/viz/adapters/d3-sankey.ts`, <640px destination stacked-bar fallback + disclosure, always-present sr-only `<table>`. Mirrors `money-flow`'s `MoneyFlowSankey` field-for-field. Unit is people.
- `district-province.ts` — authoritative `DISTRICT_TO_PROVINCE` (77→7) + `PROVINCE_ORDER`, generated from the source geometry's province field. Covered by `district-province.test.ts` (77 districts, 14/8/13/11/12/10/9 split).
...
- `server/queries.ts` (View B/C additions) — `getAbsenteeShareByPalika()` (per-palika share, joins the population denominator) and `getMigrationFlowSankey()` (province→region flow) + the pure, exported `buildMigrationFlowGraph()` (covered by `flow-graph.test.ts`).
...
- `page` at `src/app/migration/page.tsx` — async Server Component; reuses Pulse `KpiCard`; "what this shows" prose, source/confidence/unit footer (Grade A), View A chart + View C flow Sankey + View B choropleth (each with its own typed fallback, fetched in one `Promise.all`), and a disabled "Remittance by recipient district — coming soon" placeholder. Renders typed empty/error states; never throws.
