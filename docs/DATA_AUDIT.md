...
- **Historical intergovernmental fiscal transfers** — ✅ FY2078/79 + FY2079/80 RECOVERED 2026-06-08 (Stream 2): text-layer values, 753/753 reconcile to the printed `स्थानीय तह` document total to the rupee, 6,024 rows each, `npr_crore`, confidence B, `extraction_method=textlayer`. ⚠️ The remaining **6 FYs (2074/75, 2075/76, 2077/78, 2080/81, 2081/82, 2082/83-pdf) are scanned** (no usable numeric text layer) → genuine Surya-OCR territory, deferred until OCR output reconciles per the ADR-0021 gate (the harness is ready). **Surya GPU cross-check pass (populate `ocr_tracking` for the 2 ingested FYs) is pending** — the CLI's `--surya` invocation needs a small fix (the parser's `from ..` relative import doesn't resolve under bare-script invocation; needs absolute import + `sys.path` bootstrap), then a ~5–15 min/FY GPU run. The DATA itself does not depend on it (text-layer values already reconcile + ship).
