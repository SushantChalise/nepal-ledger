...
- **Category leaf mapping.** Ch.2 prints parent groups ("To be regularized", "Advance") with sub-rows. The parser emits **leaves only** (skips the "To be regularized" parent and `3.x` advance sub-rows) so the lines sum to the printed total without double-counting. `Recoverable→recoverable`, `Irregular→irregular`, `Evidences not submitted→evidence_not_submitted`, `Balance not brought forward→responsibility_not_transferred`, `Reimbursements not received→revenue_arrears`, `Advance (parent total)→advance_outstanding`.
...
- **Reconciliation gate (passes to the rupee).** Summed category lines are checked against the printed "Total irregularity" row per class; non-zero variance → `ReconciliationFailed` in `errors[]` + `status='partial'`. The 58th reconciles exactly: **Σ irregularity = NRs 104,384.3 M**; per-class federal 44,392.1 / provincial 6,499.7 / local 40,834.7 / committee 12,657.8 M; **Σ cumulative outstanding = NRs 418.85 bn**.
