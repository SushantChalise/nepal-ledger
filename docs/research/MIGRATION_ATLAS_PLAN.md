...
- **Phase 2 — data acquisition (partial ✅ 2026-06-11):** registered `fewims-migrant-deaths` (M8) + `idmc-displacement` (M6) (paused, profiles staged); **KNOMAD (M5) dropped — dysfunctional.** The **absentee-% intensity layer** (originally Phase-3) shipped early. The **`migration_permit_facts` fact-domain foundation** (M3) is built — schema + enums + migration + repo + Zod contract ([ADR-0026](../decisions/0026-migration-permit-fact-domain.md)), **foundation only**. **Remaining:** the deterministic DoFE parser (flips `dofe-labour-migration` active + populates the table); backfill remittance BoP (M4).
