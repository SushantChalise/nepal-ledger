...
- **Phase 2 — data acquisition (partial ✅ 2026-06-11):** registered `fewims-migrant-deaths` (M8) + `idmc-displacement` (M6) (paused, profiles staged); **KNOMAD (M5) dropped — dysfunctional.** The **absentee-% intensity layer** (originally Phase-3) shipped early — View B fills by share-of-population-abroad via the `Indv01_PopulationBySex` denominator. **Remaining:** activate `dofe-labour-migration` + new `migration_permit_facts` table (own enum ADR); backfill remittance BoP (M4).
- **Phase 3 — lens build-out:** ~~absentee-% intensity~~ (✅ done early), layer switcher across the other census layers, live Sankey, remittance ledger + error-catch story, "Cost of Leaving" calculator.
